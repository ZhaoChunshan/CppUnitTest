...
TEST(OurTest, __matrix__) {
    ...
    for(int t = 0; t < T; ++t){
        ...
        for(int i = 0; i < n; ++i){
            for(int j = 0; j < n; ++j)
                EXPECT_EQ(mat1[i][j], mat_res[i][j]);
        }
        infile.close();
        outfile.close();
    }
